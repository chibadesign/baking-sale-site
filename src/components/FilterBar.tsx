    ...
    <aside className="bg-white rounded-2xl shadow p-4 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => set({ category: cat })}
              className={`text-left text-xs px-3 py-2 rounded-lg transition-colors duration-150 ${filter.category === cat ? "bg-[#FFDBC9] text-[#31100C] font-semibold" : "text-gray-700 hover:bg-[#F5DED8]"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <section>
        ...
        <select value={filter.maxPrice ?? ""} onChange={e => set({ maxPrice: e.target.value === "" ? null : Number(e.target.value) })}
          className="w-full text-xs border border-[#D8C2BC] rounded-lg px-3 py-2 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#8B4513]">
      ...
      <button onClick={() => onChange({ category: "すべて", minDiscount: 0, maxPrice: null, sortBy: "discount" })}
        className="w-full text-xs text-[#8B4513] border border-[#8B4513] rounded-full py-2 hover:bg-[#FFDBC9] transition-colors duration-150">
        リセット
      </button>
    </aside>
  );
