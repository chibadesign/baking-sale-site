        ...
        <div className="flex flex-col gap-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => set({ category: cat })}
              className={`text-left text-xs px-3 py-2 rounded-lg transition-colors duration-150 ${
                filter.category === cat
                  ? "bg-[#FFF3E0] text-[#D84315] font-semibold"
            ...
            <button key={opt.value} onClick={() => set({ minDiscount: opt.value })}
              className={`text-xs px-2.5 py-1 rounded-full border transition-colors duration-150 ${
                filter.minDiscount === opt.value
                  ...
                  : "border-[#FFCCBC] text-gray-600 hover:bg-[#FFF3E0]"
              }`}>
              {opt.label}
            </button>
          ))}
